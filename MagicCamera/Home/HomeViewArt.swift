            ...
            ScrollView {
                VStack {
                    ...
                    ForEach(Array(stride(from: 0, to: DefaultsKeys.allFilters.count, by: 2)), id: \.self) { idx in
                        HStack{
                            Spacer()
                            let filter1 = DefaultsKeys.getFilterAtIndex(idx)
                            let name1 = DefaultsKeys.allFilters[idx].name
                            NavigationLink(destination: BeautyCameraView(CmUtil: self.CmUtil, fxName: name1, idx:idx)) {
                                ProImageView(name: "face.jpg", title:name1, pro: true, filter: filter1)
                            }
                            Spacer()
                            let filter2 = DefaultsKeys.getFilterAtIndex(idx+1)
                            let name2 = DefaultsKeys.allFilters[idx+1].name
                            NavigationLink(destination: BeautyCameraView(CmUtil: self.CmUtil, fxName: name2, idx:idx+1)) {
                                ProImageView(name: "face.jpg", title:name2, pro: true, filter: filter2)
                            }
                            Spacer()
                        }
                    }
